...
dilated = ndimage.binary_dilation(changed_mask, structure=struct, iterations=3)
...
# --- Size change detection via center-aligned scale search ---
def detect_size_change(ref, live, x_min, y_min, x_max, y_max, padding=60):
    img_w, img_h = live.size

    sx1 = max(x_min - padding, 0)
    sy1 = max(y_min - padding, 0)
    sx2 = min(x_max + padding, img_w)
    sy2 = min(y_max + padding, img_h)

    ref_crop = np.array(ref.crop((x_min, y_min, x_max, y_max)).convert("L")).astype(np.float32)
    live_area = np.array(live.crop((sx1, sy1, sx2, sy2)).convert("L")).astype(np.float32)

    ref_h, ref_w = ref_crop.shape
    live_h, live_w = live_area.shape

    best_score = float("inf")
    best_scale = 1.0

    # Try scaling ref_crop by various factors and see which matches live best
    for scale in np.arange(0.8, 1.5, 0.05):
        new_w = int(ref_w * scale)
        new_h = int(ref_h * scale)
        if new_w > live_w or new_h > live_h:
            continue

        scaled = np.array(
            Image.fromarray(ref_crop.astype(np.uint8)).resize((new_w, new_h), Image.LANCZOS)
        ).astype(np.float32)

        # Center-crop from live_area to match scaled size
        cy = (live_h - new_h) // 2
        cx = (live_w - new_w) // 2
        live_patch = live_area[cy:cy+new_h, cx:cx+new_w]

        score = np.abs(live_patch - scaled).mean()
        if score < best_score:
            best_score = score
            best_scale = scale

    if best_score < 20 and abs(best_scale - 1.0) > 0.03:
        dw = int(ref_w * best_scale) - ref_w
        dh = int(ref_h * best_scale) - ref_h
        return dw, dh  # positive = grew, negative = shrank
...
def detect_shift(ref, live, x_min, y_min, x_max, y_max, padding=40):
    ...
    # correlate2d with mode='valid' slides ref over the search area
...
for i in range(1, num_features + 1):
    ...
    size_change = detect_size_change(ref, live, x_min, y_min, x_max, y_max, padding=60)

    parts = []
    if shift:
        sx, sy = shift
        parts.append(f"x{'+' if sx >= 0 else ''}{sx}px  y{'+' if sy >= 0 else ''}{sy}px")
    if size_change:
        dw, dh = size_change
        parts.append(f"w{'+' if dw >= 0 else ''}{dw}px  h{'+' if dh >= 0 else ''}{dh}px")

    shift_str = "  |  ".join(parts) if parts else "no change detected"
